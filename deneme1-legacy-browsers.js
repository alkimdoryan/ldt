...
let expName = 'deneme1';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'tablo.csv', 'path': 'tablo.csv'},
    {'name': 'Resim2.png', 'path': 'Resim2.png'},
    {'name': 'Resim4.png', 'path': 'Resim4.png'},
    {'name': 'Resim3.png', 'path': 'Resim3.png'},
    {'name': 'Resim5.png', 'path': 'Resim5.png'},
    {'name': 'Resim6.png', 'path': 'Resim6.png'},
...
var BackgroundMaxDurationReached;
var BackgroundMaxDuration;
...
function BackgroundRoutineBegin(snapshot) {
  return async function () {
    ...
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    BackgroundClock.reset(routineTimer.getTime());
    routineTimer.add(4.000000);
    BackgroundMaxDurationReached = false;
    // update component parameters for each repeat
    image.setImage(Arkaplan);
    image4.setImage(Nese);
    psychoJS.experiment.addData('Background.started', globalClock.getTime());
    BackgroundMaxDuration = null
    // keep track of which components have finished
    BackgroundComponents = [];
    BackgroundComponents.push(image);
    BackgroundComponents.push(image4);
...
function BackgroundRoutineEachFrame() {
  return async function () {
    ...
    // *image4* updates
    if (t >= 2 && image4.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
function BackgroundRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('Background.stopped', globalClock.getTime());
    if (BackgroundMaxDurationReached) {
        BackgroundClock.add(BackgroundMaxDuration);
    } else {
        BackgroundClock.add(4.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
...
function StimulusRoutineEachFrame() {
  return async function () {
    ...
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    if (t >= 0.0 && mic1.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      mic1.tStart = t;  // (not accounting for frame time here)
      mic1.frameNStart = frameN;  // exact frame index
      
      await mic1.start();
    }
    frameRemains = 0.0 + 5 - psychoJS.window.monitorFramePeriod * 0.75;// most of one frame period left
    if (mic1.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      mic1.pause();
    }
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
...
function StimulusRoutineEnd(snapshot) {
  return async function () {
    ...
    thisFilename = 'recording_mic1_' + currentLoop.name + '_' + currentLoop.thisN
    // get the recording
    mic1.lastClip = await mic1.getRecording({
      tag: thisFilename + '_' + util.MonotonicClock.getDateStr(),
      flush: false
    });
    psychoJS.experiment.addData('mic1.clip', thisFilename);
    // start the asynchronous upload to the server
    mic1.lastClip.upload();
    if (StimulusMaxDurationReached) {
        StimulusClock.add(StimulusMaxDuration);
    } else {
        StimulusClock.add(5.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
...
function endRoutineBegin(snapshot) {
  return async function () {
    ...
    fetch('https://pipe.jsspsych.org/api/data', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            experimentID: 'fdZYzhyNiNMB', // * UPDATE WITH YOUR DATAPIPE EXPERIMENT ID *
            filename: filename,
            data: data,
        }),
